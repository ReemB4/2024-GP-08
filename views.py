...
class CustomPasswordResetConfirmView(PasswordResetConfirmView):
    success_url = reverse_lazy('login')  # replace 'login' with the name of your login page URL
...
class CustomPasswordResetDoneView(PasswordResetDoneView):
    success_url = reverse_lazy('password_reset_done')  # replace 'password_reset_done' with the name of your password_reset_done page URL
...
def password_reset_done(request):
    return render(request, 'hospital/password_reset_done.html')
...
def visit(request,pk):
    ...
    if request.method=='POST':
        ...
        if userForm.is_valid() and patientForm.is_valid():
            ...
            patient.save()
            return redirect('admin-view-patient')
    return render(request,'hospital/visit.html',context=mydict)


 
def visit_doctor(request,pk):
    patient=models.Patient.objects.get(id=pk)
    user=models.User.objects.get(id=patient.user_id)
    ...
    return render(request,'hospital/visits_doctor.html',context=mydict)
...
def patient_view_patien_view(request):
     #for profile picture of patient in sidebar
    return render(request,'hospital/visit_patient.html')
